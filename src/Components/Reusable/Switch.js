...
const Switch = ({items, onToggle, isToggled}) => {
    ...
    return (
        <div className='flex items-center h-8 border-solid border-blue-900 rounded-3xl text-white font-semibold border-2 relative z-[1]'>
            <div ref={item1Ref} onClick={handleToggle.bind(null, items[0])} className={`h-8 py-1 px-5 rounded-3xl cursor-pointer ${isToggled || activeText}`}>{items[0]}</div>

            <div ref={item2Ref} onClick={()=>{handleToggle(items[1])}} className={`h-8 py-1 px-5 rounded-3xl cursor-pointer ${isToggled && activeText}`}>{items[1]}</div>
